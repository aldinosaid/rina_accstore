<?php
    ...
    function do_print($dataNota) {
        try {
            ...
            $printer -> text("RINA ACCESORIES STORE\n");
            ...
            // Set Summary order
            $printer -> text("TOTAL " . $dataNota['total'] . "\n");
            $printer -> text("DISC " . "0%\n");
            $printer -> text("BAYAR " . $dataNota['bayar'] . "\n");
            $printer -> text("KEMBALI " . $dataNota['kembali'] . "\n");
            // Set Contact Person
            $printer -> setJustification(Printer::JUSTIFY_CENTER);
            $printer -> text("** TERIMAKASIH **\n");
            $printer -> text("SUDAH BERBELANJA\n");
            $printer -> text("DI RINA ACCSTORE\n");
